...
<body>
	...
	<div id="wrapper">
		...
		<div id="main">
			...
			<div id="category_wrapper_wrapper">
				<form action="RecommendSelectCon" method="post">
					<div class="category_wraper">
						<span>지역</span> <input type="text" list="city" class="category"
							name="city">
						<datalist id="city">
         ...
         <datalist id = "a_L_category1">
          ...
          -->
					<div class="category_wraper">
						<span>카테고리</span> <input type="text" list="a_m_category1"
							class="category" name="a_m_category1">
								...
								<td>
									<div class="popUp" id="popUp<%=i%>">
										<article>
											...
											<div class="recInfo">
												<strong>상호명 : </strong>
												<%=recommend.get(i).getAc_name()%>
												<br> <strong>주소 : </strong>
												<%=recommend.get(i).getAc_addr()%><br> <strong>관련정보
													: </strong> <a href="<%=recommend.get(i).getAc_rel()%>"
													target="_blank">홈페이지 바로가기</a> <a
													href="<%=recommend.get(i).getAc_rev()%>" target="_blank">
													블로그리뷰 </a><br>
											</div>
 ...
 %> <br>
											...
											<div class="wishInput">
												...
												<form action="WishlistRecoDelCon" method="post">
													<button type="submit" class="h1" name="wishDelete"
														value="<%=recommend.get(i).getAc_id()%>">
												...
												<form action="WishListInputCon" method="post">
													<button type="submit" class="h0" name="a_idToWish"
														value="<%=recommend.get(i).getAc_id()%>">
														<img src="./images/heart0.png" alt="heart0" class="heart">
													</button>
												</form>
												<%
												}
													...
													<td class="revinfo">
														<form action="ReviewDeleteCon" method="post" class="form">
															<div class="revDate">
																...
																%>
																<button type="submit" class="revDel" name="rev_id"
																	value="<%=recommend.get(i).getAc_id()%>">삭제</button>
															</div>
															<div class="revUser">
															...
															<div>
																평점 :
																<%=Review.get(j).getRe_score()%>
																<input type="password" class="revPass" name="revPass"
																	placeholder="비밀번호"> <input type="hidden"
																	name="rev_nick"
																	value="<%=Review.get(j).getRe_nick()%>">
															</div>
															<br>
												...
												<table border="1" class=revWrite>
													<tr>
														...
														<td><select name="score">
															...
															value="<%=recommend.get(i).getAc_id()%>" name="re_id">
															<input type="hidden"
															value="<%=recommend.get(i).getAc_name()%>"
															name="classname"></td>
									...
									<div class="recInfo">
										...
										<%=recommend.get(i).getAc_addr()%><br> <a
											href="<%=recommend.get(i).getAc_rel()%>" target="_blank">홈페이지
											바로가기</a><br>
									</div>
									<div class="wishInput">
									...
									<div class="recReviewWish">
										<img alt="star" src="./images/star.png" class="star"> <strong>
											평점 : <%
 if (scoreAvg != null) {
 	if (scoreAvg.get(i) == null) {
 %> <%=0%> 점 <%
 ...
 %> <br>
							...
							<tr id="pagebox">
								<td>
									...
									%> <a href="Recommend.jsp?pageNum=<%=i%>">[<%=i%>]
								</a> <%
 }
 } // for end
 ...
 %> <a href="Recommend.jsp?pageNum=<%=startPage + 10%>">[다음]</a>
				...
				<div id="recMap">
					...
					%>
					<input type="hidden"
						value="<%=recommend.get(i).getAc_name()%>,<%=recommend.get(i).getAc_wi()%>,<%=recommend.get(i).getAc_ky()%>,<%=recommend.get(i).getAc_addr()%>,<%=recommend.get(i).getAc_img()%>,<%=recommend.get(i).getAc_rev()%>,<%=recommend.get(i).getAc_rel()%>"
						class="locmarker">
					<%
					}
					%>
					<input type="hidden" value="<%=locmarker.get(0).getAC_WI()%>"
						class="wi"> <input type="hidden"
						value="<%=locmarker.get(0).getAC_KY()%>" class="ky">
					<div id="map">
						...
						<script>
							...
							var mapContainer = document
									.getElementById("recMap"), mapOption = {
								center : new kakao.maps.LatLng(locArray[0][1],
										locArray[0][2]),
								level : 4,
